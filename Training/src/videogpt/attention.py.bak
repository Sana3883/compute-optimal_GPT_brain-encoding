...
from src.videogpt.utils import view_range, tensor_slice
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, cond, decode_step, decode_idx):
        ...
        if self.use_frame_cond:
            ...
            cond_data = cond["frame_cond"] if "frame_cond" in cond else cond["emb_cond"]
            if self.training:
                h = checkpoint(self.enc_attn, h, cond_data, cond_data, decode_step, decode_idx)
            else:
                h = self.enc_attn(h, cond_data, cond_data, decode_step, decode_idx)
    ...
    def get_ops(self):
        ...
        if self.shape not in SparseAttention.ops:
            ...
            sparse_dot_sdd_nt = MatMul(
                sparsity_layout, self.sparsity_config.block, "sdd", trans_a=False, trans_b=True
            )

            sparse_dot_dsd_nn = MatMul(
                sparsity_layout, self.sparsity_config.block, "dsd", trans_a=False, trans_b=False
            )
...
class RightShift(nn.Module):
    ...
    def forward(self, x, decode_step):
        ...
        x = x.flatten(start_dim=1, end_dim=-2)  # (b, t_d * h_d * w_d, embd_dim)
